...
def previous_install(args, path):
    ...
    for blockdevice_outside in [f"{path}1", f"{path}p1"]:
        ...
        pmb.helpers.mount.bind_file(
            args, blockdevice_outside, args.work + "/chroot_native" + blockdevice_inside
        )
        try:
            label = pmb.chroot.root(
                args,
                ["blkid", "-s", "LABEL", "-o", "value", blockdevice_inside],
                output_return=True,
            )
        except RuntimeError:
            logging.info(
...
def mount_disk(args, path):
    ...
    pmb.helpers.mount.bind_file(args, path, args.work + "/chroot_native/dev/install")
    if previous_install(args, path):
        if not pmb.helpers.cli.confirm(
            args, "WARNING: This device has a" " previous installation of pmOS." " CONTINUE?"
        ):
            raise RuntimeError("Aborted.")
    else:
        if not pmb.helpers.cli.confirm(args, f"EVERYTHING ON {path} WILL BE" " ERASED! CONTINUE?"):
            raise RuntimeError("Aborted.")


def create_and_mount_image(args, size_boot, size_root, size_reserve, split=False):
    """
    ...
    for img_path, size_mb in images.items():
        logging.info(f"(native) create {os.path.basename(img_path)} " f"({size_mb})")
    ...
    for img_path, mount_point in mount_image_paths.items():
        logging.info("(native) mount " + mount_point + " (" + os.path.basename(img_path) + ")")
        pmb.install.losetup.mount(args, img_path)
        device = pmb.install.losetup.device_by_back_file(args, img_path)
        pmb.helpers.mount.bind_file(args, device, args.work + "/chroot_native" + mount_point)
...
def create(args, size_boot, size_root, size_reserve, split, disk):
    ...
    """
    pmb.helpers.mount.umount_all(args, args.work + "/chroot_native/dev/install")
    if disk:
