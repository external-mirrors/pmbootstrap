...
def package(args, pkgname, reason="", dry=False):
    ...
    if int(apkbuild["pkgrel"]) != pkgrel_new:
        raise RuntimeError(
            "Failed to bump pkgrel for package '"
            + pkgname
            + "'. Make sure that there's a line with exactly the"
            " string '" + old + "' and nothing else in: " + path
        )
...
def auto_apkindex_package(args, arch, aport, apk, dry=False):
    ...
    for depend in depends:
        ...
        providers = pmb.parse.apkindex.providers(args, depend, arch, must_exist=False)
        if providers == {}:
            ...
            if depend.startswith("so:") or not pmb.helpers.pmaports.find(args, depend, False):
