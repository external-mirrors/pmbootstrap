...
def get_upstream_remote(args, name_repo):
    ...
    raise RuntimeError(
        "{}: could not find remote name for URL '{}' in git" " repository: {}".format(
            name_repo, url, path
        )
    )
...
def parse_channels_cfg(args):
    ...
    if args.config_channels:
        ...
        stdout = pmb.helpers.run.user(args, command, args.aports, output_return=True, check=False)
        try:
...
def pull(args, name_repo):
    ...
    if branch not in branches_official:
        logging.warning(
            "{} not on one of the official branches ({}), skipping" " pull!" "".format(
                msg_start, ", ".join(branches_official)
            )
        )
        return -1
...
def get_topdir(args, path):
    ...
    return pmb.helpers.run.user(
        args, ["git", "rev-parse", "--show-toplevel"], path, output_return=True, check=False
    ).rstrip()
...
def get_files(args, path):
    ...
    ret = []
    files = pmb.helpers.run.user(args, ["git", "ls-files"], path, output_return=True).split("\n")
    files += pmb.helpers.run.user(
        args, ["git", "ls-files", "--exclude-standard", "--other"], path, output_return=True
    ).split("\n")
    for file in files:
