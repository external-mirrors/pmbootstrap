...
def copy_to_buildpath(args, aport_dir, suffix: Suffix=Suffix.native()):
    # Sanity check
    # Support either an absolute path or a package name
    if not aport_dir.startswith("/"):
        aport_dir = pmb.helpers.pmaports.find(args, aport_dir)
    logging.info(f"Copying {aport_dir} to buildpath")
    if not os.path.exists(aport_dir + "/APKBUILD"):
    ...
    # Copy aport_dir contents with resolved symlinks
    pmb.helpers.run.root(args, ["mkdir", "-p", build])
    for entry in os.listdir(aport_dir):
        ...
        if entry in ["src", "pkg"]:
            ...
            continue
        pmb.helpers.run.root(args, ["cp", "-rL", f"{aport_dir}/{entry}", f"{build}/{entry}"])
