...
def modify_apkbuild(args, pkgname, aport):
    ...
    pmb.helpers.run.user(args, ["cp", "-r", apkbuild_path, args.work + "/aportgen"])
...
def run_abuild(args, pkgname, arch, apkbuild_path, kbuild_out):
    ...
    if build_output != "":
        if os.path.islink(chroot + "/mnt/linux/" + build_output) and os.path.lexists(
            chroot + "/mnt/linux/" + build_output
        ):
            pmb.chroot.root(args, ["rm", "/mnt/linux/" + build_output])
        pmb.chroot.root(args, ["ln", "-s", "/mnt/linux", build_path + "/src"])
    pmb.chroot.root(args, ["ln", "-s", kbuild_out_source, build_path + "/src" + build_output])
