...
def copy_git_repo_to_chroot(args, topdir):
    ...
    pmb.helpers.run.user(args, ["tar", "-cf", tarball_path, "-T", f"{tarball_path}.files"], topdir)

    ci_dir = "/home/pmos/ci"
    pmb.chroot.user(args, ["rm", "-rf", ci_dir])
    pmb.chroot.user(args, ["mkdir", ci_dir])
    pmb.chroot.user(args, ["tar", "-xf", "/tmp/git.tar.gz"], working_dir=ci_dir)
...
def run_scripts(args, topdir, scripts):
    ...
    for script_name, script in scripts.items():
        ...
        if "native" in script["options"]:
            rc = pmb.helpers.run.user(args, [script_path], topdir, output="tui")
            continue
        else:
            ...
            env = {"TESTUSER": "pmos"}
            rc = pmb.chroot.root(
                args, [script_path], check=False, env=env, working_dir="/home/pmos/ci", output="tui"
            )
        if rc:
