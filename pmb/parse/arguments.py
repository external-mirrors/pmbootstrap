...
def arguments_export(subparser):
    ...
    ret.add_argument(
        ...
        default="/tmp/postmarketOS-export",
        nargs="?",
    )
    ret.add_argument(
        "--odin",
...
def arguments_sideload(subparser):
    ...
    ret.add_argument("--arch", help="use a different architecture than the one" " set in init")
    ret.add_argument(
        "--install-key",
...
def arguments_repo_missing(subparser):
    ...
    ret.add_argument(
        "--arch", choices=pmb.config.build_device_architectures, default=pmb.config.arch_native
    )
    ret.add_argument(
...
def arguments_status(subparser):
    ret = subparser.add_parser("status", help="quick health check for the work dir")
    ret.add_argument(
        "--details", action="store_true", help="list passing checks in detail, not as summary"
    )
    return ret
...
def package_completer(prefix, action, parser=None, parsed_args=None):
    ...
    packages = set(
        package for package in pmb.helpers.pmaports.get_list(args) if package.startswith(prefix)
    )
    return packages
...
def arguments():
    ...
    parser.add_argument(
        ...
        "--mirror-alpine",
        dest="mirror_alpine",
        help="Alpine Linux mirror, default: " + pmb.config.defaults["mirror_alpine"],
        metavar="URL",
    )
    ...
    parser.add_argument("-p", "--aports", help="postmarketos aports (pmaports) path")
    parser.add_argument(
        "-t",
        "--timeout",
    ...
    update.add_argument(
        "--arch", default=None, choices=arch_choices, help="only update a specific architecture"
    )
    update.add_argument(
        "--non-existing",
        action="store_true",
    ...
    build.add_argument(
        ...
        + arch_native
        + " or first available architecture in"
        " APKBUILD)",
    )
    ...
    config.add_argument(
        "name",
        nargs="?",
        help="variable name, one of: " + ", ".join(sorted(pmb.config.config_keys)),
