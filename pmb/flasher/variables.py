...
def variables(args, flavor, method):
    ...
    if method.startswith("fastboot"):
        ...
        _partition_rootfs = (
            args.deviceinfo["flash_fastboot_partition_rootfs"]
            or args.deviceinfo["flash_fastboot_partition_system"]
            or "userdata"
        )
        _partition_vbmeta = args.deviceinfo["flash_fastboot_partition_vbmeta"] or None
        _partition_dtbo = args.deviceinfo["flash_fastboot_partition_dtbo"] or None
    # Require that the partitions are specified in deviceinfo for now
    elif method.startswith("rkdeveloptool"):
        _partition_kernel = args.deviceinfo["flash_rk_partition_kernel"] or None
        _partition_rootfs = (
            args.deviceinfo["flash_rk_partition_rootfs"]
            or args.deviceinfo["flash_rk_partition_system"]
            or None
        )
        _partition_vbmeta = None
        _partition_dtbo = None
    elif method.startswith("mtkclient"):
        _partition_kernel = args.deviceinfo["flash_mtkclient_partition_kernel"] or "boot"
        _partition_rootfs = args.deviceinfo["flash_mtkclient_partition_rootfs"] or "userdata"
        _partition_vbmeta = args.deviceinfo["flash_mtkclient_partition_vbmeta"] or None
        _partition_dtbo = args.deviceinfo["flash_mtkclient_partition_dtbo"] or None
    else:
        _partition_kernel = args.deviceinfo["flash_heimdall_partition_kernel"] or "KERNEL"
        _partition_rootfs = (
            args.deviceinfo["flash_heimdall_partition_rootfs"]
            or args.deviceinfo["flash_heimdall_partition_system"]
            or "SYSTEM"
        )
        _partition_vbmeta = args.deviceinfo["flash_heimdall_partition_vbmeta"] or None
        _partition_dtbo = args.deviceinfo["flash_heimdall_partition_dtbo"] or None
    ...
    vars = {
        ...
        "$RECOVERY_ZIP": "/mnt/buildroot_"
        + args.deviceinfo["arch"]
        + "/var/lib/postmarketos-android-recovery-installer"
        "/pmos-" + args.device + ".zip",
        "$UUU_SCRIPT": "/mnt/rootfs_"
        + args.deviceinfo["codename"]
